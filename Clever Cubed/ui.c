...
void ui_render_sheet(WINDOW* win, PlayerSheet* sheet, UIState* s) {
    ...
    wattron(win, COLOR_PAIR(CP_TITLE) | A_BOLD);
    if (g_bot_type != BOT_NONE && sheet->id == 1)
        mvwprintw(win, 0, 2, " %s SHEET ", bot_name(g_bot_type));
    else if (sheet->id == 0 && g_bot_type != BOT_NONE)
        mvwprintw(win, 0, 2, " PLAYER SHEET ");
    else
...
void ui_render_actions_bar(WINDOW* win, PlayerSheet* sheet, UIState* s, int player_id) {
    ...
    switch (s->mode) {
        ...
        case UIMODE_PASSIVE_PICK:
            mvwprintw(win, 0, 12, "< >:move  ENTER:pick  a:any nr  q:quit");
            break;
...
static void apply_bonus_as_die(UIState* s, GameState* game, PlayerSheet* sheet,
    ...
    if (bonus_type == BONUS_YELLOW) {
        ...
        // Blue bonus: the player chose a value, add white die value.
        // Search ALL dice for white (including platter) since during passive
        // or bonus context all dice may already be on platter.
        int white_val = 0;
        for (int i = 0; i < 6; i++) {
            if (game->dice[i].color == DIE_COLOR_WHITE && game->dice[i].is_available) {
...
// Called after every successful mark to check if cv.c set a pending colored bonus
static void check_and_route_bonus(UIState* s, GameState* game, PlayerSheet* sheet) {
    if (sheet->pending_bonus_type == 0) return;
    int bt = sheet->pending_bonus_type;
    sheet->pending_bonus_type = 0;
    s->pending_bonus_type  = bt;
    s->pending_bonus_value = -1;
    s->mode = UIMODE_BONUS_CHOOSE_VALUE;
    char buf[128];
    const char* cname = (bt==BONUS_YELLOW)?"Yellow":(bt==BONUS_TURQUOISE)?"Turquoise":
                        (bt==BONUS_BLUE)?"Blue":(bt==BONUS_BROWN)?"Brown":"Pink";
    snprintf(buf, sizeof(buf), "Bonus! Choose a value (1-6) for %s:", cname);
    ui_set_status(s, buf, false);
}
...
bool ui_handle_input(UIWindows* w, UIState* s, GameState* game, int viewer_player) {
    ...
    case UIMODE_ACTIVE_ROLL: {
        if (ch == KEY_LEFT || ch == 'h') {
                ...
                if (pick_die(game, s->cursor_die, &picked)) {
                    ...
                    if (eff_color == DIE_COLOR_WHITE) {
                        ...
                        if (total == 7) {
                            ...
                            bool ok = mark_blue(sheet, total, true);
                            if (ok) { check_and_route_bonus(s, game, sheet); }
                            if (ok && s->mode == UIMODE_BONUS_CHOOSE_VALUE) break;
                            if (ok) {
                                move_lower_dice_to_platter(game, picked.value);
                        ...
                        if (ok) { check_and_route_bonus(s, game, sheet); }
                        if (ok && s->mode == UIMODE_BONUS_CHOOSE_VALUE) { /* bonus fired, wait */ }
                        else if (ok) {
                            char buf[64];
            ...
            if (sheet->actions.any_number_charges > 0) {
                s->mode = UIMODE_ANY_NUMBER_COLOR;
                ui_set_status(s, "Any Number: choose color  y=yellow  t=turquoise  b=blue  r=brown  p=pink", false);
            } else {
    ...
    case UIMODE_ACTIVE_MARK: {
        ...
        if (ch == KEY_LEFT || ch == 'h') {
            ...
            if (ok) { check_and_route_bonus(s, game, sheet); }
            if (ok && s->mode == UIMODE_BONUS_CHOOSE_VALUE) { s->cursor_col = 0; break; }
            if (ok) {
                s->cursor_col = 0;
                char buf[64];
            ...
            if (!d->on_silver_platter) {
                ...
                if (c == DIE_COLOR_YELLOW) {
                    ...
                    if (ok) {
                        ...
                        check_and_route_bonus(s, game, sheet);
                        if (s->mode != UIMODE_BONUS_CHOOSE_VALUE) {
                            ui_set_status(s, "Yellow marked (passive).", false);
                            s->passive_context = false;
                            s->mode = UIMODE_ACTIVE_ROLL;
                        }
                    } else {
    ...
    case UIMODE_BONUS_CHOOSE_VALUE: {
        if (ch >= '1' && ch <= '6') {
            ...
            // If apply_bonus fired another sub-mode (turquoise, brown etc) leave it.
            // Otherwise end cleanly respecting passive context.
            if (s->mode == UIMODE_ACTIVE_ROLL || s->mode == UIMODE_PASSIVE_PICK) {
                end_sub_mode(s, game, sheet);
            }
        } else if (ch == 27) {
            s->passive_context = false;
    ...
    case UIMODE_ANY_NUMBER_COLOR: {
        DieColor dc = DIE_COLOR_NONE;
        if      (ch == 'y' || ch == 'Y') dc = DIE_COLOR_YELLOW;
        else if (ch == 't' || ch == 'T') dc = DIE_COLOR_TURQUOISE;
        else if (ch == 'b' || ch == 'B') dc = DIE_COLOR_BLUE;
        else if (ch == 'r' || ch == 'R') dc = DIE_COLOR_BROWN;
        else if (ch == 'p' || ch == 'P') dc = DIE_COLOR_PINK;
        else if (ch == 27) {
            s->mode = UIMODE_ACTIVE_ROLL;
        ...
        if (dc != DIE_COLOR_NONE) {
            // Find the die of this color on the board (available, not on platter)
            int found_idx = -1;
            for (int i = 0; i < 6; i++) {
                if (game->dice[i].color == dc &&
            ...
            if (found_idx < 0) {
                char abuf[128];
                snprintf(abuf, sizeof(abuf), "No available %s die on the board!", ui_color_name(dc));
                ui_set_status(s, abuf, true);
            } else {
                // any_number_die_index = physical die index (for original value + platter)
                // selected_die_index   = BONUS_* constant (for marking)
                s->any_number_die_index = found_idx;
                s->selected_die_index   = (dc == DIE_COLOR_YELLOW)    ? BONUS_YELLOW :
                                          (dc == DIE_COLOR_TURQUOISE)  ? BONUS_TURQUOISE :
                                          (dc == DIE_COLOR_BLUE)       ? BONUS_BLUE :
                                          (dc == DIE_COLOR_BROWN)      ? BONUS_BROWN : BONUS_PINK;
                s->mode = UIMODE_ANY_NUMBER_VALUE;
                char abuf[128];
                snprintf(abuf, sizeof(abuf),
                    "Any Number %s (die=%d): pick value 1-6:",
                    ui_color_name(dc), game->dice[found_idx].value);
                ui_set_status(s, abuf, false);
            }
        }
        break;
    }
    ...
    case UIMODE_ANY_NUMBER_VALUE: {
        if (ch >= '1' && ch <= '6') {
            ...
            if (slot < 0) {
                ...
                // Mark bar slot and decrement charge
                Die dummy; dummy.value = 0;
                bool ok = use_any_number_action(sheet, &dummy, val);
                if (ok) {
                    // Platter cutoff = original die's actual rolled value, NOT the any-number value
                    int orig_die_val = game->dice[s->any_number_die_index].value;
                    move_lower_dice_to_platter(game, orig_die_val);
                    // Pick (consume) the actual die from the board
                    Die picked_an; pick_die(game, s->any_number_die_index, &picked_an);
                    // Check for bar completion bonus
                    if (sheet->actions.any_number_used == 7) {
                        char buf[128];
                        snprintf(buf, sizeof(buf),
                        ...
                        // Mark the area using the chosen value (not original die value)
                        int chosen_color = s->selected_die_index; // BONUS_* constant
                        apply_bonus_as_die(s, game, sheet, chosen_color, val, false);
                        if (s->mode == UIMODE_ACTIVE_ROLL || s->mode == UIMODE_PASSIVE_PICK) {
                            char buf[128];
                            snprintf(buf, sizeof(buf), "Any Number: %s %d applied (die was %d).",
                                     ui_color_name(picked_an.color), val, orig_die_val);
                            ui_set_status(s, buf, false);
                        }
                    }
                } else {
