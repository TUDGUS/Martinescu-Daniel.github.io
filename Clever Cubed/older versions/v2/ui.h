...
// Per-player action bar colors
#define CP_PLAYER1      18  // white on blue
#define CP_PLAYER2      19  // white on red
#define CP_PLAYER3      20  // black on green
#define CP_PLAYER4      21  // white on magenta
...
typedef enum {
    ...
    UIMODE_ANY_NUMBER_COLOR,    // pick color for any-number action
...
typedef struct {
    ...
    int         pending_bonus_value; // value to use with the bonus (-1 = ask player)
    int         any_number_die_index; // index of die chosen for any-number action
    ...
    bool        passive_context;    // true when sub-mode entered from passive pick
} UIState;
