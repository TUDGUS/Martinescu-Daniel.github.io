...
void ui_render_top(WINDOW* win, GameState* game, UIState* s) {
    ...
    const char* mode_str = "";
    switch (s->mode) {
        case UIMODE_ACTIVE_ROLL:          mode_str = "[ SELECT A DIE ]"; break;
...
void ui_render_dice(WINDOW* win, GameState* game, UIState* s) {
    ...
    int lx = 2 + 6 * 9 + 2;
    wattron(win, COLOR_PAIR(CP_DEFAULT));
    mvwprintw(win, 1, lx, "< > : select die    &:reroll");
    mvwprintw(win, 2, lx, "ENTER: pick die     AN:any-number");
    mvwprintw(win, 3, lx, "a   : use action    ^v^: fox");
...
void ui_render_actions_bar(WINDOW* win, PlayerSheet* sheet, UIState* s) {
    werase(win);
    wattron(win, COLOR_PAIR(CP_ACTION));
    for (int r = 0; r < WIN_ACTIONS_H; r++)
        for (int c = 0; c < getmaxx(win); c++)
            mvwaddch(win, r, c, ' ');

    wattron(win, A_BOLD);
    mvwprintw(win, 0, 1, "ACTIONS");
    wattroff(win, A_BOLD);
    ...
    switch (s->mode) {
        ...
        case UIMODE_USE_ACTION:
            mvwprintw(win, 0, 12, "r:reroll  n:any-number  e:extra-die  ESC:cancel");
            break;
    ...
    int x = 1;
    // Reroll
    wattroff(win, COLOR_PAIR(CP_ACTION));
...
void ui_render_all(UIWindows* w, UIState* s, GameState* game, int viewer_player) {
    if (s->mode == UIMODE_GAME_OVER) {
        ...
        ui_render_actions_bar(w->actions, &game->players[viewer_player], s);
...
static int next_any_die(int cur, int dir) {
    return (cur + dir + 6) % 6;
}
...
bool ui_handle_input(UIWindows* w, UIState* s, GameState* game, int viewer_player) {
        ...
        if (ch == KEY_LEFT || ch == 'h') {
            ...
            if (!d->is_available || d->on_silver_platter) {
                ...
                if (pick_die(game, s->cursor_die, &picked)) {
                    ...
                    // After picking and platter-moving, check if any dice remain.
                    // Store this now -- color handlers below may update chosen_dice_count.
                    bool no_dice_left = (count_pickable_dice(game) == 0);
                    // Determine which mode to enter for marking
                    ...
                    if (eff_color == DIE_COLOR_WHITE) {
                        ...
                        if (total == 7) {
                            ...
                            if (ok) {
                                ...
                                // Mark failed: undo the pick so player can choose a different die
            ...
            s->mode = UIMODE_USE_ACTION;
            ui_set_status(s, "Use action: r=reroll  n=any-number  e=extra-die  ESC=cancel", false);
        } else if (ch == 'q') {
            return false;
        }
        break;
    ...
    case UIMODE_ACTIVE_MARK: {
        ...
        if (ch == KEY_LEFT || ch == 'h') {
            if (s->cursor_brown > 0) s->cursor_brown--;
        } else if (ch == KEY_RIGHT || ch == 'l') {
            if (s->cursor_brown < 11) s->cursor_brown++;
        } else if (ch == '\n' || ch == KEY_ENTER) {
            bool ok = mark_brown(sheet, last->value, s->cursor_brown);
            if (ok) {
                char buf[64];
                snprintf(buf, sizeof(buf), "Brown: marked index %d.", s->cursor_brown);
                ui_set_status(s, buf, false);
                { if (game->chosen_dice_count >= 3 || count_pickable_dice(game) == 0) try_end_active_turn(s, game, sheet); else s->mode = UIMODE_ACTIVE_ROLL; }
            } else {
    ...
    case UIMODE_ACTIVE_TURQUOISE: {
        ...
        if (ch == KEY_UP || ch == 'k') {
            ...
            bool ok = mark_turquoise(sheet, last->value, s->cursor_row, true);
            if (ok) {
                char buf[64];
        ...
        if (ch == KEY_LEFT || ch == 'h') {
            ...
            if (!d->on_silver_platter) {
                ...
                if (c == DIE_COLOR_YELLOW) {
                    ...
                    s->mode = UIMODE_ACTIVE_TURQUOISE;
                    ui_set_status(s, "Turquoise (passive): choose rows with ?? ENTER", false);
                    ...
                    s->mode = UIMODE_ACTIVE_MARK;
                    char buf[64];
                    snprintf(buf, sizeof(buf), "Brown %d (passive): navigate then ENTER", d->value);
                    ui_set_status(s, buf, false);
                    ...
                    char wbuf[64];
                    snprintf(wbuf, sizeof(wbuf),
                        "White die (joker) rolled %d (passive)! Choose area: y t b r p", d->value);
                    ui_set_status(s, wbuf, false);
                }
            }
        } else if (ch == 'a') {
            s->mode = UIMODE_USE_ACTION;
            ui_set_status(s, "Use action: n=any-number  e=extra-die  ESC=cancel", false);
        } else if (ch == 'n') {
    ...
    case UIMODE_BLUE_SEVEN: {
        if (ch == 'l' || ch == 'L') {
            ...
            ui_set_status(s, placed ? "Blue: 7 placed on left." : "No space on left!", !placed);
            if (placed) { if (game->chosen_dice_count >= 3 || count_pickable_dice(game) == 0) try_end_active_turn(s, game, sheet); else s->mode = UIMODE_ACTIVE_ROLL; }
        } else if (ch == 'r' || ch == 'R') {
            bool placed = false;
            for (int i = 7; i <= 12; i++) {
                if (sheet->blue_area[i] == 0) {
            ...
            ui_set_status(s, placed ? "Blue: 7 placed on right." : "No space on right!", !placed);
            if (placed) { if (game->chosen_dice_count >= 3 || count_pickable_dice(game) == 0) try_end_active_turn(s, game, sheet); else s->mode = UIMODE_ACTIVE_ROLL; }
        }
        break;
    }
    ...
    case UIMODE_BONUS_CHOOSE_VALUE: {
        if (ch >= '1' && ch <= '6') {
            int val = ch - '0';
            use_color_bonus(sheet, s->pending_bonus_type, val);
            char buf[64];
            snprintf(buf, sizeof(buf), "Bonus applied: value %d.", val);
            ui_set_status(s, buf, false);
            s->mode = UIMODE_ACTIVE_ROLL;
        }
        break;
    ...
    case UIMODE_BONUS_CHOOSE_COLOR: {
        ...
        if (bonus_type != -1) {
            ...
            if (s->pending_bonus_value > 0) {
                // White die joker: value already known, apply immediately
                use_color_bonus(sheet, s->pending_bonus_type, s->pending_bonus_value);
                char wbuf[64];
                snprintf(wbuf, sizeof(wbuf), "White die applied as value %d to area %d.",
                    s->pending_bonus_value, s->pending_bonus_type);
                ui_set_status(s, wbuf, false);
                s->pending_bonus_value = 0;
                s->mode = (game->chosen_dice_count >= 3)
                          ? UIMODE_PASSIVE_PICK : UIMODE_ACTIVE_ROLL;
            } else {
                // ? bonus: ask player for value
                s->mode = UIMODE_BONUS_CHOOSE_VALUE;
...
// ===============================================================================
//  INLINE VALUE PROMPT (used for bonus inputs when needed from C)
// ===============================================================================
