...
void init_player_sheet(PlayerSheet* sheet, int id) {
    ...
    sheet->actions.reroll_charges=1;
    sheet->actions.any_number_charges=1;
    sheet->actions.extra_die_charges=1;
    sheet->actions.foxes_unlocked=1;
...
void calculate_final_scores(GameState* game) {
    for (int i=0; i<game->num_players; i++) {
        PlayerSheet* sheet=&game->players[i];
        int total=calculate_yellow_score(sheet)+
                   calculate_turquoise_score(sheet)+
                   calculate_blue_score(sheet)+
                   calculate_brown_score(sheet)+
                   calculate_pink_score(sheet)+
                   (sheet->actions.foxes_unlocked*5);
        sheet->score=total;
    }
}
